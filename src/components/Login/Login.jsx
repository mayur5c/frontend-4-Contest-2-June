...
// After successful authentication it will save the id and the received data to the redux state and it
// will redirect us to the profile page
...
import { profileUserDataSelector } from "../Profile/profileSlice";
...
export default function Login() {
  ...
  let profileUserData = useSelector(profileUserDataSelector);
  ...
  return (
    <>
      <div className="h-screen flex items-center justify-center bg-gradient-to-r from-indigo-500 from-10% via-sky-500 via-30% to-emerald-500 to-90%">
        <form className="text-xl font-[500] text-black flex flex-col rounded-3xl w-1/3 p-12 backdrop-blur-md bg-white/30 h-[80%] gap-8 md:w-1/2 sm:w-[90%] sm:h-[90%] sm:p-8 xs:p-4 tall:w-[60%]">
          ...
          <div className="mt-4">
            <input
              type="submit"
              value="Login"
              className="self-start w-1/3 rounded-lg h-12 bg-slate-800 text-blue-50 cursor-pointer inline-block"
